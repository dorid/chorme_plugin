...
<li style='color:green;'><strong>最后操作时间</strong>：<span>--</span></li></div>\
    ...
    function reloadCode() {
        ...
        for (var i = 0; i < arrCookie.length; i++) {
            var arr = arrCookie[i].split("=");
            cookie.push(arr[0] + "=" + arr[1] + "\";");
        }
        IECode += cookie.join("");
        utility.get("https://127.0.0.1:8443/getAuthCode?cookie=" + IECode, "", "jsonp");
        /*结束验证码*/
    }
    ...
    function setAuthCode(data){
        $("#randCodeTxt").val(data.code);
        if (statusShown && document.getElementById("randCodeTxt").value.length == 4) {
            checkOrderInfo();
        }
    }
